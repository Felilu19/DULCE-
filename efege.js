...
async function starts() {
	...
	Fg.on('qr', () => {
		console.log(color('[LUZAICK]','aqua'), color("Escanee el codigo QR para conectarse...", "yellow"))
	})
	fs.existsSync('./session/FG98.json') && Fg.loadAuthInfo('./session/Luzaick.json')
Fg.on('connecting', () => {
...
Fg.on('open', () => {
        ...
        console.log(color('[LUZAICK]','aqua'), color(`Conectado`, "aqua"))
        start('')
    })
...
//-------------𝗦𝗧𝗜𝗖𝗞𝗘𝗥 𝗠𝗔𝗞𝗘𝗥 ---------
			function addMetadata(packname, author) {	
				if (!packname) packname = '•°❤︎⋆𝐋𝐔𝐙𝐀𝐈𝐂𝐊-𝐁𝐎𝐓⋆❤︎•°'; if (!author) author = pushname ;	
...
if (!isGroup && isCmd) console.log(color('[LUZAICK]','aqua'), "Pv", color(command, "blue"), "de", (sender.split('@')[0]),  args.length)
...
case 'creador':
    ...
    nomor = '50242725900@s.whatsapp.net'
...
case 'enlinea':
  ...
  if (!isGroup) return reply(group())
			    ...
			    Fg.sendMessage(from, 'En linea:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n` + `\n*©POWERED BY LUZAICK ┃ ᴮᴼᵀ*`, text, { quoted: mek,
